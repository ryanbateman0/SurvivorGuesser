    ...
    return(
        <div class="flex-col relative z-10 inset-x-0 top-16 h-16">
            <div class="flex justify-center">
                <input class="w-xl flex-initial border  focus:border-gray-800 border-gray-600 p-4 text-left" 
            ...
            <ul class="p-1 justify-center inline-block w-full md:w-3xl  bg-white border border-gray-100">
                {filteredSuggestions.map(filteredSuggestion => (
                    <li class="m-0.5 p-2 text-black border bg-white hover:bg-gray-100" key={filteredSuggestion} onClick={() => handleSuggestionClick(filteredSuggestion)}>{filteredSuggestion}</li>
                ))}
            </ul>
            )}
        </div>
